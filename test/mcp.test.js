...
test("bundled stdio server negotiates MCP and advertises the complete tool set", async () => {
  ...
  assert.match(stderr, /^\[seo-crawl-audit:mcp\] MCP server 0\.10\.0 is running on stdio\. Waiting for client requests; press Ctrl\+C to stop\.\n$/);
...
test("repository development entrypoint starts the MCP server", async () => {
  ...
  assert.equal(exitCode, 0, stderr);
  assert.equal(stdout, "");
  assert.match(stderr, /MCP server 0\.10\.0 is running on stdio/);
});
