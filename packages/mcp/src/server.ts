...
const common = {
  ...
  maxPages: z.number().int().positive().max(50000).optional(),
...
export function createServer(root = workspaceRoot()): McpServer {
  ...
  register(server, "seo_audit_scan", "Run SEO scan", "Scan a site locally and save a SnapshotV2 plus an HTML report. Defaults to 100 pages and a resumable checkpoint. Full sitemap scans above 5,000 URLs require explicit confirmation.", { ...common, fullSitemap: z.boolean().optional(), confirmLargeScan: z.boolean().optional(), output: z.string().optional(), report: z.string().optional(), checkpoint: z.string().optional(), resume: z.boolean().optional() }, (input, signal) => scanTool(context(signal), input));
