...
export async function createLocalUiServer(options: LocalUiOptions = {}): Promise<LocalUiServer> {
  ...
  const startScan = async (input: Record<string, unknown>) => {
    ...
    try {
      ...
      const completeReportData = { ...reportData, ...(history ? { history } : {}) };
      await Promise.all([writeSnapshot(snapshotPath, result.snapshot), writeReport(reportPath, completeReportData)]);
