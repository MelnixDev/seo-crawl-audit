...
function thresholdBlocks(severity: Severity, threshold: string): boolean {
...
async function run(): Promise<void> {
  ...
  const result = await scan(config, {
    onEvent(event) {
      if (event.type === "progress" && event.completed && (event.completed === event.total || event.completed % 25 === 0)) {
  ...
  await writeFile(reportPath, renderReport(reportData), "utf8");
...
run().catch((error: unknown) => action.setFailed(error instanceof Error ? error.message : String(error)));
